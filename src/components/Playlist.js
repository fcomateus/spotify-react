...
export default function Song() {
  ...
  async function carregarMusicas(playlists) {
    const playlistUnica = playlists.find((item) => item._id == id);
    ...
    const listaMusicas = await res.data.filter((musica) =>
      playlistUnica.musicas.includes(musica._id)
    );
  ...
  async function modificaListaMusicas(id) {
    const listaMusicasSalvas = musicas.filter((musica) => musica._id === id);
    const listaMusicas = musicas.map((musica) => musica._id);
    if (listaMusicasSalvas.length) {
      ...
      await api.patch(`/playlists/${playlist._id}`, {
        ...playlist,
        musicas: [...novaListaMusicasSalvas],
      });
    } else {
