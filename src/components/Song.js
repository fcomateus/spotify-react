...
export default function Song() {
  const { id } = useParams();
  const playlist = playlistsMock.find((item) => item.id == id);
  ...
  function toggle(indice) {
    ...
    listaMusicas[indice] = {...listaMusicas[indice], tocando: !listaMusicas[indice].tocando};
  ...
  function monitorarDuracao(indice) {
    ...
    listaMusicas[indice].musica.addEventListener(
      'timeupdate',
      (time) => {
        let listaMusicas = [...musicas];
        ...
        listaMusicas[indice] = {...listaMusicas[indice], tocando: true, tempoAtual: listaMusicas[indice].musica.currentTime};
    ...
    listaMusicas[indice].musica.addEventListener(
      ...
      (time) => {
        ...
        listaMusicas[indice] = {...listaMusicas[indice], tocando: false, tempoAtual: 0};
    ...
    <div className="music-player">
      ...
      <ul className="player-list">
        {playlist.musicas.map((musica, indice) => (
          <li key={musica.id}>
              <img className="list-cover" src={playlist.Capa} alt='capa_musica' />
              ...
              <div className="main-control">
                <div
                  ...
                  style={{
                    backgroundImage: `url("/player/${musicas[indice]?.tocando ? 'pause' : 'play'}.svg")`,
                  }}
                ...
                <div className="btn _timeline">
                  <span className="current-time">
                    {musicas[indice]?.musica ? `${
                        `${new Date(musicas[indice].musica.currentTime * 1000).getUTCMinutes()}`.padStart(2, '0')
                      }:${
                        `${new Date(musicas[indice].musica.currentTime * 1000).getUTCSeconds()}`.padStart(2, '0')
                      }` : '00:00'
                    }
                  </span>
                  
                  <span className="timescope">
                    <span className="timescope-dot" style={{
                      left: `${musicas[indice]?.musica ? ((musicas[indice].musica.currentTime * 100) / musicas[indice].musica.duration): 0}%`
                    }}></span>
                  <div style={{
                    position: 'absolute',
                    top: 0,
                    left: 0,
                    display: 'block',
                    width: `${musicas[indice]?.musica ? ((musicas[indice].musica.currentTime * 100) / musicas[indice].musica.duration): 0}%`,
                    height: '100%',
                  ...
                  <span className="end-time">
                    {
                      musicas[indice]?.musica.duration ? `${
                        new Date(musicas[indice].musica.duration * 1000).getUTCMinutes()
                      }:${
                        new Date(musicas[indice].musica.duration * 1000).getUTCSeconds()
                      }`
                      : '--:--'
                    }
                  </span>
